...
class TheTvDbSource extends Source {
  ...
  getAllSeriesEpisodes(seriesName, seasonId, options, cb) {
    ...
    this._handleEpisodes(seriesName, (this._isValidNumber(seasonId) ? 'SeasonNumber = "' + seasonId + '"' : null),
        (episodes, error) => {
      if(error) {
        cb(null, error);
      } else {
  ...
  getSeriesEpisode(seriesName, seasonId, episodeId, cb) {
    if (!episodeId) {
      cb(null, 'seriesName and episodeId are required arguments.');
    }
    this._handleEpisodes(seriesName, 'EpisodeNumber = "' + episodeId + '"' +
        (this._isValidNumber(seasonId) ? ' and SeasonNumber = "' + seasonId + '"' : ''), (episodes, error) => {
      if(error) {
        cb(null, error);
      } else {
        episodes.forEach(episode => cb(this._episodeToObject(episode)));
